...
public class SimpleExample {
    ...
    private static Connection getConnection() throws ClassNotFoundException,SQLException{
        ...
        String keytabPath = "./hive.service.keytab";
        String principalName = "hive/node1.hde.h3c.com@HDE.H3C.COM";
        org.apache.hadoop.conf.Configuration conf = new org.apache.hadoop.conf.Configuration();
        conf.set("hadoop.security.authentication", "Kerberos");

        //获取krb5.conf配置
        if(System.getProperty("os.name").toLowerCase().startsWith("win")) {
            System.setProperty("java.security.krb5.conf", SimpleExample.class.getClassLoader().getResource(krb5ConfPath).getPath());
        }

        //进行身份认证
        try{
            UserGroupInformation.setConfiguration(conf);
            UserGroupInformation.loginUserFromKeytab(principalName,SimpleExample.class.getClassLoader().getResource(keytabPath).getPath());
